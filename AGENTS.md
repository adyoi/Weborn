...
- **UI Addon Store & Apps diracik ulang**: kartu featured "Weborn Core" (badge, glow accent, tombol Buka Panel/Apps Monitor/Kelola, status `running`); grid addon inti dikecualikan dari kartu biasa (`{% for a in addons if not a.core %}`); halaman Create Apps dipangkas — hapus Web Server/Web Server Config/PHP-FPM Config/Node Config/tombol check-port (guard & modal lama dibuang, endpoint `api/check-port` & `api/free-port` tetap), Create Web API Workers default 2, field Domain jadi dropdown dari tabel `domains` (kind=domain, aktif) yang dipakai sebagai `server_name` nginx (`slug = domain atau nama app`).
- **Preview config addon graceful**: `render_config()` return `None` bila template `.j2` tidak ada (hanya ~21 template tersedia; netdata/glances/weborn-engine/dll belum punya) → halaman detail tampilkan daftar nilai field, bukan "(gagal render template)".
- **Terminal logs anti-wrap**: semua xterm (Apps Monitor modal/detail, app logs, panel detail) pakai `convertEol: true` + fit berlapis (rAF ×2, transitionend, retry 60/250ms, catch) + `container.innerHTML=''` sebelum buka ulang — perbaiki spasi/tab antar-baris kacau.
- **Group filter Apps Type**: chips halaman Application = `Semua, WSGI, ASGI, JavaScript, PHP` (`?type=wsgi|asgi|js|php`). Sumber: `APP_TYPE_GROUPS` (label) + `APP_TYPES_IN_GROUP` (app_type per group; `js` = nodejs/deno/bun, `php` = php/laravel) di `config.py`; URL app_type lama (`?type=nodejs`) tetap jalan lewat fallback `{type}` di `routers/apps.py`. `static` tidak masuk grup mana pun → hanya muncul di "Semua".
- **Runtime Deno & Bun**: masuk `RUNTIMES`/`APP_TYPES`/`FRAMEWORKS` (fresh/hono/oak + elysia/hono/express/bun.serve). Stub `LANG_STUB` per runtime (`main.ts`); framework tanpa stub khusus kini jatuh ke `LANG_STUB.get(language)` (fallback di `create()`), scaffold keyword bertambah `fresh.deno.dev` & `bun create`. `install_cmd` (belum dijalankan) memasang biner ke `/usr/local/bin`. Framework PHP baru: **Spiral** & **Yii3** (app_type `php`, composer create-project).
- **Edit Aplikasi diperkaya** (`/apps/{id}/edit` POST): Port & Direktori bisa diedit — busy-check port (get_app_by_port + `_port_busy`), update `.env` (PORT/APP_DIR) + unit (WorkingDirectory/EnvironmentFile) + `systemctl enable/restart`. **User OS** kini diseragamkan: `weborn-apps` (dedicated, shared — dibuat sekali, `useradd -r -M -d /var/www -s /bin/false` bila belum ada) atau `www-data`; user OS lama (`weborn-<slug>`) di-**migrasi** ke `weborn-apps` saat save (chown -R); validasi di POST tolak selain `weborn-apps`/`www-data` (400). **Nama app bisa diedit** (`name`, unique via `get_app_by_name` kecuali id sendiri) → rename unit `.service`, `.env` GUNICORN_SOCK, nginx conf (re-apply bila conf `sites-available/{slug}.conf` lama ada). **Perintah Start** = `ExecStart` sbg User OS dengan env `.env` (`Restart=on-failure` 5 dtk); app PHP/static (serve_only) dilayani Nginx — unit systemd stubs opsional, **checkbox `systemd_enabled` default NONAKTIF** (unchecked → hapus stub + `systemctl disable`; `unit_applies = (not serve_only) or want_unit`; `_detect_pm` diimport lokal di POST). Bahasa/Framework tampil di Info card (bukan form). Halaman apps: card 1 Check Port, card 2 field `Host (Domain / IP)` default `{{ server_ip }}` (dari `routers/domains._get_server_ip`) + Check Port — fetch `/apps/api/check-port?port=N`.
- **Netdata terpasang (Debian 13 / addon weborn-engine)**: port 19999 active, API `http://127.0.0.1:19999/api/v1/info` (bukan `/v1/info` → 404); addon belief port & unit `netdata`. Rintangan yang sudah dilewati: paket `netdata` TIDAK ada di Debian trixie; repo `repo.netdata.cloud` redirect 404 ke `/no-such-repo`; kickstart ABORTED karena `/etc/apt/sources.list.d/netdata.list` rusak (Err:6 → `apt-get update` gagal). Fix: `rm` list + keyring, lalu `get.netdata.cloud/kickstart.sh --non-interactive --dont-wait --stable-channel --disable-cloud` → repoconfig dari **`repository.netdata.cloud`** terpasang → agent OK.
- Rencana user berikutnya: addon **Weborn Webmail** (bagian dari keluarga "Weborn Core"; Roundcube sudah ada — kemungkinan wrapper/responsive shell, belum dibangun; jangan dibangun tanpa dicek).
