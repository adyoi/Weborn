...
- **Edit Aplikasi diperkaya** (`/apps/{id}/edit` POST): Port & Direktori bisa diedit — busy-check port (get_app_by_port + `_port_busy`), update `.env` (PORT/APP_DIR) + unit (WorkingDirectory/EnvironmentFile) + `systemctl enable/restart`. **User OS** switch `weborn-<slug>` ↔ `www-data` (chown -R baru). **Perintah Start** = `ExecStart` yang dijalankan systemd sbg User OS dari direktori app dgn env `.env` (`Restart=on-failure` 5 dtk); app PHP/static hanya label (dilayani Nginx). **Unit systemd opsional utk PHP/static** (serve_only): checkbox `systemd_enabled` → hapus/cipta ulang stub `ExecStart=/bin/true` + disable/enable; info card badge `DIBUTUHKAN`/`OPTIONAL (stub)`/`NONAKTIF` (GET menghitung `unit_exists` via executor). Form fields baru: `port`, `dir_path`, `os_user`, `systemd_enabled`; `_detect_pm` harus diimport lokal di POST (jangan lupa).
