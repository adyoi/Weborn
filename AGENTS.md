...
- **DKIM TXT panjang (420 char)** — value di tabel `email_dns.html` di-**slice** (60 char + ellipsis) dengan tooltip `title` nilai utuh; nilai lengkap tampil di kartu "DNS Records yang Dibutuhkan" (kotak `break-all`). Regenerasi kunci: POST `/email/dns/dkim/generate` (opendkim-genkey → rename `{domain}.weborn.*` → root:root 0700 → upsert TXT `weborn._domainkey.{domain}` → restart opendkim) — record DNS di-update otomatis.
