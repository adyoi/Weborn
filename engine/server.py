...
class SecureHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._content_compressed = False
    ...
    def load_htaccess(self, path):
        """Muat dan cache file .htaccess dengan parsing, termasuk warisan dari direktori induk"""
        if path in self.htaccess_cache:
            return self.htaccess_cache[path]

        rules = self._parse_htaccess(path)

        # Warisi aturan dari direktori induk hingga base directory
        if path != self.base_directory:
            parent = os.path.dirname(path)
            if parent.startswith(self.base_directory):
                rules = self._merge_rules(self.load_htaccess(parent), rules)

        self.htaccess_cache[path] = rules
        return rules

    def _empty_rules(self, path):
        return {
            'directory': path,
            'auth': {},
            'redirects': [],
    ...
    def _merge_rules(self, parent, child):
        merged = {
            'directory': child['directory'],
            'auth': {**parent['auth'], **child['auth']},
            'redirects': parent['redirects'] + child['redirects'],
            'rewrites': parent['rewrites'] + child['rewrites'],
            'error_pages': {**parent['error_pages'], **child['error_pages']},
            'deny': parent['deny'] + child['deny'],
            'allow': parent['allow'] + child['allow'],
            'requires': child['requires'] if child['requires'] else parent['requires'],
            'custom': {**parent['custom'], **child['custom']},
            'headers': {**parent['headers'], **child['headers']}
        }
        if 'rewrite_enabled' in child:
            merged['rewrite_enabled'] = child['rewrite_enabled']
        elif 'rewrite_enabled' in parent:
            merged['rewrite_enabled'] = parent['rewrite_enabled']
        return merged

    def _parse_htaccess(self, path):
        """Parsing dan cache file .htaccess untuk satu direktori"""
        if path in self._raw_htaccess_cache:
            return self._raw_htaccess_cache[path]

        htaccess_path = os.path.join(path, '.htaccess')
        rules = self._empty_rules(path)
        
        if os.path.isfile(htaccess_path):
            try:
                with open(htaccess_path, 'r', encoding='utf-8-sig') as f:
                    lines = f.readlines()
        ...
        self._raw_htaccess_cache[path] = rules
        return rules
                ...
                try:
                    ...
                    if userfile:
                        if not os.path.isabs(userfile):
                            userfile = os.path.join(rules.get('directory', self.base_directory), userfile)
            ...
            for rewrite in rules['rewrites']:
                try:
                    ...
                    if re.match(rewrite['pattern'], path):
                        replacement = re.sub(r'\$(\d+)', r'\\\1', rewrite['substitution'])
                        new_path = re.sub(rewrite['pattern'], replacement, path)
                        flags = rewrite.get('flags', '')
                        ...
                        # Terapkan rewrite internal dan hentikan jika [L]
                        self.path = new_path
                        if 'L' in flags:
                            return 2
                        path = new_path
                ...
                if 'regex' in redirect:
                    try:
                        if re.match(redirect['regex'], path):
                            replacement = re.sub(r'\$(\d+)', r'\\\1', redirect['to'])
                            target = re.sub(redirect['regex'], replacement, path)
    ...
    def handle_error_page(self, code, message=None):
        ...
        try:
            ...
            # Cegah rekursi tak terbatas saat penanganan error
    ...
    def end_headers(self):
        ...
        for header, value in self.security_headers.items():
            if header.lower() not in self._response_headers:
                self.send_header(header, value)
                
        # Tambahkan header GZIP hanya jika konten benar-benar dikompres
        if self.gzip_enabled and self._content_compressed:
            self.send_header('Content-Encoding', 'gzip')
        
        # Tangkap panjang konten untuk logging
        if self.path != '/favicon.ico':
            for chunk in getattr(self, '_headers_buffer', []):
                if chunk.lower().startswith(b'content-length:'):
                    self.log_data['bytes_sent'] = int(chunk.split(b':', 1)[1])
                    break
    ...
    def handle_request(self):
        ...
        try:
            ...
            rewrite_result = self.apply_rewrites(rules)
            if rewrite_result == 1:
                return  # Pengalihan rewrite terkirim
            if rewrite_result == 2:
                path = self.translate_path(self.path)
                if not path:
                    return
    ...
    def send_head(self):
        ...
        try:
            ...
            self.send_response(self._status_override if self._status_override is not None else 200)
    ...
    def create_logger(filename, max_bytes, backup_count):
        ...
        def emit(msg):
            record = logging.LogRecord(
                name='weborn', level=logging.INFO, pathname=__file__, lineno=0,
...
def run_server(port=8000, directory=os.getcwd(), 
    ...
    SecureHTTPRequestHandler.directory = real_base
