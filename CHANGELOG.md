...
### Added
- **Runtime JavaScript baru**: **Deno** & **Bun** masuk `RUNTIMES`/`APP_TYPES`/`FRAMEWORKS` — Runtime select di form Create Apps kini menawarkan Node.js, Deno, Bun dengan framework stabil masing-masing (Deno: Fresh, Hono, Oak, std/http; Bun: Elysia, Hono, Express, Bun.serve); stub starter `main.ts` per runtime & `install_cmd` pemasangan sistem-wide
- **Framework PHP baru**: **Spiral** & **Yii3** (`composer create-project spiral/app` / `yiisoft/app`) di daftar framework PHP
- **Group filter Apps Type**: chips di halaman Application kini `Semua, WSGI, ASGI, JavaScript, PHP` berbasis `APP_TYPE_GROUPS` + `APP_TYPES_IN_GROUP` (backend: `?type=js` mencakup app_type nodejs/deno/bun, `?type=php` mencakup php/laravel; URL app_type lama seperti `?type=nodejs` tetap didukung)
- **Weborn Core di Addon Store**: kartu featured untuk addon inti Weborn (badge "Weborn Core", border/glow accent, status `running`, meta category/ports/version, tombol Buka Panel / Apps Monitor / Kelola) + badge "Weborn Core" di tiap kartu addon inti pada grid & halaman detail; legend status
- **Dropdown Domain di Apps**: field Domain di kedua form Create (Web Apps/Frontend & Web API/Backend) kini `select` berisi domain aktif dari tabel `domains` (kind=domain); nilai dipakai sebagai `server_name` nginx (kosong = pakai nama app)
...
- **Edit Aplikasi diperkaya** (`/apps/{id}/edit`): **Port** & **Direktori** kini dapat diedit (busy-check port, update `.env` PORT/APP_DIR + unit `WorkingDirectory`/`EnvironmentFile` + restart); **User OS** dapat dialihkan antara user dedicated `weborn-<slug>` dengan `www-data` (nginx/php-fpm) — kepemilikan direktori di-chown; **Perintah Start** diberi penjelasan fungsi & contoh; **unit systemd opsional untuk app PHP/static** — toggle hapus/re-create stub `ExecStart=/bin/true` (lengkap dengan `systemctl enable/disable`); info card menampilkan badge `DIBUTUHKAN` / `OPTIONAL (stub)` / `NONAKTIF`
- **Edit Aplikasi div2 (rebuild)**: **Nama bisa diedit** (unique — tolak bila dipakai app lain; unit `weborn-<slug>.service`, socket `.env` & config nginx ikut di-rename + re-apply); **User OS diseragamkan** jadi `weborn-apps` (dedicated) | `www-data` (nginx/php-fpm) — user OS lama di-migrasi ke `weborn-apps` saat save (tanpa membuat user baru); **unit systemd default nonaktif** untuk app PHP/static (checkbox tanpa checked); **Bahasa & Framework dipindah** dari form kiri ke tabel Info Aplikasi; halaman di-redesign (form kiri + aksi cepat, info kanan)
- **Check Port di halaman Apps**: tombol **Check** di card "Create Web Apps / Frontend" (port) dan card "Create Web API / Backend" (port) — memanggil `GET /apps/api/check-port?port=N` (telusuri port kosong / dipakai app / proses lain, warna ok/err)
- **Host (Domain / IP) di Create Web API**: field baru di card "Create Web API / Backend" yang default-nya IP server (`_get_server_ip()`); bila diisi dipakai sebagai `server_name` nginx (slug = value domain), validasi IP/hostname diperluas (`0.0.0.0, 192.168.1.7, example.com`)

### Changed
- **Halaman Application diformat ulang**: form "Create Web Apps / Frontend" kini ringkas (hapus Web Server, Web Server Config, PHP-FPM Config, Node Config, dan tombol check-port); form "Create Web API / Backend" hapus Web Server, **Workers default 2**; guard port-client dihapus (endpoint `api/check-port` & `api/free-port` tetap tersedia)
...
### Fixed
- **Preview config addon "gagal render template"**: `render_config()` kini mengembalikan `None` bila template Jinja tidak ada; halaman addon menampilkan fallback (daftar nilai field) alih-alih error render
- **Terminal logs tidak sejajar**: xterm di Apps Monitor (modal & detail) dan app logs kini `convertEol: true`, fit berlapis setelah modal benar-benar tampil (rAF + transitionend + retry), kontainer dibersihkan sebelum dibuka ulang — spasi/tab per baris tidak lagi kacau (wrap)
