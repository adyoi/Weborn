...
Config domain/proxy di-generate dari data panel (DB) lalu ditulis ke
CONFIG_DIR untuk ditinjau, dan (di mode local) di-relayout via nginx -t + reload.
"""
...
class NginxManager:
    ...
    async def _deploy(self, name: str):
        """Tulis config ke /etc/nginx + symlink + test + reload (mode local/wsl)."""
        if self.executor.mode not in ("local", "wsl"):
            return
        src = self.conf_dir / f"{name}.conf"
        if not src.exists():
            return
        # nginx belum terinstall? lewati dengan tenang.
        ok = await self.executor.run("bash", "-c", "test -d /etc/nginx/sites-available && echo yes || echo no")
        ...
        await self.executor.run("bash", "-c",
                                f"ln -sf {remote} /etc/nginx/sites-enabled/{name}.conf")
        r = await self.executor.run("nginx", "-t")
        if r.ok:
            await self.executor.run("nginx", "-s", "reload")
        else:
            await self.executor.run("bash", "-c",
                                    f"rm -f /etc/nginx/sites-enabled/{name}.conf")

    async def _remove(self, name: str):
        if self.executor.mode not in ("local", "wsl"):
            return
        await self.executor.run("bash", "-c",
            f"rm -f /etc/nginx/sites-available/{name}.conf /etc/nginx/sites-enabled/{name}.conf; "
    ...
    def apply_domain(self, domain: str, root: str, proxy_target: str | None = None,
                     ssl: bool = False):
        conf = self.build_site_conf(domain, root, proxy_target, ssl)
